...
# Удаляем контейнеры с такими же именами, если они существуют
echo -e "${YELLOW}🧹 Очистка существующих контейнеров...${NC}"
docker rm -f visitor_db visitor_app 2>/dev/null || true

# Удаляем старые образы для пересборки (опционально)
echo -e "${YELLOW}🗑️  Очистка старых образов...${NC}"
...
echo "   docker-compose down -v     # Остановка + удаление volumes"
echo "   docker-compose ps          # Статус контейнеров"
echo "   docker-compose restart     # Перезапуск"
echo "   ./stop.sh                  # Полная остановка и очистка"
echo
